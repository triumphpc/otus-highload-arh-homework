...
Примеры  запросов:
```bash
curl --location 'http://localhost:8080/api/v1/dialog/2/list' \
...
[
    ...
    },
    {
        ...
        "receiver_id": "1",
        "text": "Привет, привет",
        "sent_at": "2025-06-24 18:00:11",
        "is_own": false
    },
    {
        "sender_id": "2",
        "receiver_id": "1",
        "text": "Привет, привfsdfdfет",
        "sent_at": "2025-06-24 18:06:36",
        "is_own": false
    },
    {
        "sender_id": "1",
        "receiver_id": "2",
        "text": "Давай дружить",
        "sent_at": "2025-06-24 18:09:45",
        "is_own": true
    }
]
```

По стратегии реагирования на эффект Леди Гаги - предлагал выше - выделать таких точечно в отдельный шард и делать ребаланс. 

Какие еще могут быть подходы для борьбы с эффектом Леди Гаги:
Хэш от пары (sender_id, recipient_id) для диалогов - позволит равномерно распределить, но тогда JOIN будет в разные шарды - задержка.
